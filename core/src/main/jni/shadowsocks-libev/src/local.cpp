...
server_handshake(EV_P_ ev_io *w, buffer_t *buf)
{
    ...
    LOGE("port port port acl:%d port:%d port:%s\n", acl,atoi(port), port);
...
server_stream(EV_P_ ev_io *w, buffer_t *buf)
{
    ...
    if (!remote->send_ctx->connected) {
        ...
        if (!fast_open || remote->direct) {
            ...
            LOGE("start connect:%d, errno:%d--%d\n", r, errno, CONNECT_IN_PROGRESS);
...
remote_send_cb(EV_P_ ev_io *w, int revents)
{
    ...
    server_t *server              = remote->server;
    printf("start connect:remote_send_cb fd=%d, connected=%d\n", server->fd, remote_send_ctx->connected);
    ...
    if (!remote_send_ctx->connected) {
        ...
        printf("start connect: remote_send_cb fd=%d, connected=%d, r=%d\n", server->fd, remote_send_ctx->connected, r);
        if (r == 0) {
              ...
              int direct)
{
    ...
    printSocketAddress((struct sockaddr_storage *)remote_addr);
...
main(int argc, char **argv)
{
    ...
    LOGE("password:%s, method:%s, key:%s\n", password, method, key);
    ...
    LOGE("aaaaaaaaaaaaaaaaaaa: %d, mode:%d\n", __LINE__,mode);
    mode = 0;
    ...
    if (mode != UDP_ONLY) {
		...
		LOGE("aaaaaaaaaaaaaaaaaaa: launch_or_create %d, mode:%d\n", __LINE__,mode);
#else
        listenfd = create_and_bind(local_addr, local_port);
		LOGE("aaaaaaaaaaaaaaaaaaa: create_and_bind\n");
#endif
        if (listenfd == -1) {
            FATAL("bind() error");
        }
    ...
    LOGE("aaaaaaaaaaaaaaaaaaa:%d, user=%p\n", __LINE__, user);
#ifdef HAVE_LAUNCHD
    if (local_port == NULL)
